...
import logging
import os
import baselines.doom.position_tests as position_tests
import matplotlib.pyplot as plt
import io
import gym


class DoomEnvironment(DoomGame):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=self.screen_size + (3,))
        ...
        self.render = True
        self.set_render_hud(self.render)
        self.set_render_minimal_hud(self.render)  # If hud is enabled
        self.set_render_crosshair(self.render)
        self.set_render_weapon(self.render)
        self.set_render_decals(self.render)
        self.set_render_particles(self.render)
        self.set_render_effects_sprites(self.render)
        self.set_render_messages(self.render)
        self.set_render_corpses(self.render)
    ...
    def seed(self, seed=np.random.randint(100000000)):
        super(DoomEnvironment, self).set_seed(seed)
    ...
    def step(self, action_index, epoch_step=True):
        ...
        super(DoomEnvironment, self).make_action(action, self.act_skip)
        if epoch_step:
            self.epoch_steps += 1
