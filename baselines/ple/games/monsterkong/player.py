...
class Player(Person):
    ...
    def __init__(self, raw_image, position, width, height):
        ...
        # ###add by huhao: we can change here instead of accmulate 3 steps
    ...
    def continuousUpdate(self, wallGroupList, ladderGroupList, laddersAbove=range(15)):
        # Only gets run when the player is not on the ladder
        if self.onLadder == 0 or len(laddersAbove) == 0:
            ...
            if self.isJumping:
                ...
                if wallsCollided:
                    ...
                    if wallsCollided[0].getPosition()[1] > self.getPosition()[1]:
                        # wallsize/2 and charsize/2 and +1
                        self.isJumping = 0
