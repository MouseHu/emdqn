...
# def build_act_ib(make_obs_ph, model_func, z_noise, num_actions, scope="deepq", reuse=None):
...
#     model_func: (tf.Variable, int, str, bool) -> tf.Variable
...
#
#         code,reconstruction = model_func(observations_ph.get(), z_noise, num_actions, scope="q_func")
#
#
...
def build_train_simhash(make_obs_ph, model_func, num_actions, optimizer,
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if ib or vae:
            ...
            z_noise_vae = tf.placeholder(tf.float32, [None, 512], name="z_noise_vae")
        ...
        if emdqn:
            ...
            total_loss += 0.1 * tf.reduce_mean(importance_weights_ph * U.huber_loss(qec_error))
