...
class LRU_KNN_UCB_GPU(object):
    def __init__(self, capacity, z_dim, env_name, action, mode="mean", num_actions=6):
        self.action = action
    ...
    def peek(self, key, value_decay, action=-1, modify=False):
        ...
        dist, ind = knn_cuda.knn(np.array([key]), np.transpose(self.states[:self.curr_capacity]), 1)
        dist, ind = np.transpose(dist), np.transpose(ind)
        ind = ind[0][0]
        if dist[0][0] < 1e-2:
            # print("peek success")
    ...
    def knn_value(self, key, knn):
        ...
        dist, ind = knn_cuda.knn(key, np.transpose(self.states[:self.curr_capacity]), knn)
        dist, ind = np.transpose(dist), np.transpose(ind)
        coeff = np.exp(dist[0])
    ...
    def act_value(self, key, knn):
        ...
        if self.curr_capacity == 0:
            ...
            return values, actions, counts, exact_refer
