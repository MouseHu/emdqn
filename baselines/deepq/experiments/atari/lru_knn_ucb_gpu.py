...
import gc
from baselines.deepq.experiments.atari.bruteforce_knn import BruteForceKNN
...
class LRU_KNN_UCB_GPU(object):
    def __init__(self, capacity, z_dim, env_name, mode="mean"):
        ...
        # self.states = tf.zeros((capacity, z_dim), dtype=tf.float32)
        self.q_values_decay = np.zeros(capacity)
        self.count = np.zeros(capacity)
        self.lru = np.zeros(capacity)
        self.curr_capacity = 0
        self.tm = 0.0
        self.tree = BruteForceKNN(buffersize=self.capacity,dimension=z_dim,X=self.states)
        self.addnum = 0
        self.buildnum = 256
    ...
    def peek(self, key, value_decay, modify):
        ...
        dist, ind = self.tree.kneighbors(np.array([key]), n_neighbors=1)
        ind = ind[0][0]
    ...
    def knn_value(self, key, knn):
        ...
        dist, ind = self.tree.kneighbors(np.array([key]), n_neighbors=knn)
        coeff = np.exp(dist[0])
    ...
    def add(self, key, value_decay):
        if self.curr_capacity >= self.capacity:
            ...
            change_op = self.states[old_index].assign(key)
            with tf.Session as sess:
                print(sess.run(change_op))
    ...
    def update_kdtree(self):
        ...
        # self.tree = KDTree(self.states[:self.curr_capacity])
        self.tree = NearestNeighbors(algorithm="buffer_kd_tree",
                        tree_depth=9,
                        plat_dev_ids={0:[0]},
                        verbose=1)
        # self.tree = NearestNeighbors(algorithm="kd_tree",
        #                              n_jobs=4,
        #                              verbose=False)
        # self.tree = NearestNeighbors(algorithm="brute",
        #                  plat_dev_ids={0:[0]},
        #                  verbose=1)
        self.tree.fit(self.states[:self.curr_capacity])
        self.build_tree = True
        self.build_tree_times += 1
        if self.build_tree_times == 5:
            self.build_tree_times = 0
            gc.collect()
