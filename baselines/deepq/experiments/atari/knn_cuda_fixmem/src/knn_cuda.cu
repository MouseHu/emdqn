...
__global__ void cuComputeDistanceGlobal( float* A, int wA,
  ...
  // Loop parameters
  begin_A = dim*BLOCK_DIM* blockIdx.y;
  begin_B = dim*BLOCK_DIM* blockIdx.x;
  step_A  = BLOCK_DIM;
  step_B  = BLOCK_DIM;
  end_A   = begin_A + (dim-1);

    // Conditions
  int cond0 = (begin_A/dim + tx < wA); // used to write in shared memory
  int cond1 = (begin_B/dim + tx < wB); // used to write in shared memory & to computations and to write in output matrix
  int cond2 = (begin_A/dim+ ty < wA); // used to computations and to write in output matrix

  // Loop over all the sub-matrices of A and B required to compute the block sub-matrix
  for (int a = begin_A, b = begin_B; a <= end_A; a += step_A, b += step_B) {
    // Load the matrices from device memory to shared memory; each thread loads one element of each matrix
    if (a-begin_A+ty < dim){
      shared_A[ty][tx] = (cond0)? A[a + dim*tx + ty] : 0;
      shared_B[ty][tx] = (cond1)? B[b + dim*tx + ty] : 0;
    }
    else{
      shared_A[ty][tx] = 0;
      shared_B[ty][tx] = 0;
  ...
  if (cond2 && cond1)
    AB[(begin_A/dim + ty) * wB + begin_B/dim + tx] = ssd;
}
...
/**
  ...
  * @param AB    pointer on the matrix containing the wA*wB distances computed
  */
__global__ void cuComputeDistanceGlobalInverse( float* A, int wA,
    float* B, int wB, int dim,  float* AB){
...
void add_cuda(int address_num, int place,float* ref_vector,int dims){
  unsigned int size_of_float = sizeof(float);
  //printf("add cuda %f %d\n",ref_vector[0],dims*place);
  cudaMemcpy((theAddress[address_num].ref_dev + dims*place),&ref_vector[0],
    dims *size_of_float, cudaMemcpyHostToDevice);
}
void knn_cuda_fix_ref(int address_num, float* dist_host, int* ind_host, float* query_host,int query_width, int k, int ref_width, int dims){
  ...
  cudaMemcpy(theAddress[address_num].query_dev, &query_host[0],
      query_width * dims * size_of_float, cudaMemcpyHostToDevice);
  // Grids ans threads
  dim3 g_16x16(query_width/16, ref_width/16, 1);
  dim3 t_16x16(16, 16, 1);
  if (query_width%16 != 0) g_16x16.x += 1;
  ...
  // Kernel 1: Compute all the distances
  cuComputeDistanceGlobal<<<g_16x16,t_16x16>>>(theAddress[address_num].ref_dev, ref_width,
    theAddress[address_num].query_dev, query_width, dims, theAddress[address_num].dist_dev);

  // Kernel 2: Sort each column
  cuInsertionSort<<<g_256x1,t_256x1>>>(theAddress[address_num].dist_dev, theAddress[address_num].ind_dev,
      query_width, ref_width, k);

  // Kernel 3: Compute square root of k first elements
  cuParallelSqrt<<<g_k_16x16,t_k_16x16>>>(theAddress[address_num].dist_dev, query_width, k);

  // Memory copy of output from device to host
  cudaMemcpy(&dist_host[0], theAddress[address_num].dist_dev,
      query_width * k *size_of_float, cudaMemcpyDeviceToHost);
...
void knn_cuda_fix_ref_conditional(int address_num, float* dist_host, int* ind_host, float* query_host,bool* musk, int query_width, int k, int ref_width, int dims){
  ...
  cudaMemcpy(theAddress[address_num].musk_dev, &musk[0],
        ref_width * size_of_bool, cudaMemcpyHostToDevice);
  // Grids ans threads
  dim3 g_16x16(query_width/16, ref_width/16, 1);
  dim3 t_16x16(16, 16, 1);
  if (query_width%16 != 0) g_16x16.x += 1;
  ...
  //modify
  dim3 g_m_256x1(ref_width/256, 1, 1);
  dim3 t_m_256x1(256, 1, 1);
  if (ref_width%256 != 0) g_m_256x1.x += 1;

   cuParallelMusk<<<g_m_256x1,t_m_256x1>>>(theAddress[address_num].dist_dev, theAddress[address_num].musk_dev,ref_width);
  // Kernel 2: Sort each column
  cuInsertionSort<<<g_256x1,t_256x1>>>(theAddress[address_num].dist_dev, theAddress[address_num].ind_dev,
      query_width, ref_width, k);
