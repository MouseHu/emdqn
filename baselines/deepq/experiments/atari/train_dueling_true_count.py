    ...
    with U.make_session(4) as sess:
        ...
        def update_ec(sequence):
            ...
            for seq in reversed(sequence):
                ...
                qs,inrs = zip(*[ec_buffer[i].peek(z,exRtd,inRtd,False) for i in range(env.action_space.n)])
                q = qs[a]
                inr = np.max(inrs)
        ...
        while True:
            ...
            if num_iters % args.learning_freq == 0 and len(replay_buffer) > args.batch_size * (
                ...
                inputs = [obses_t, obses_tp1, np.array(actions_t).squeeze(), rewards, dones, np.ones_like(dones), values_t,
                          values_tp1]
                if args.imitate:
