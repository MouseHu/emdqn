...
class LRU_KNN_COUNT_GPU_FIXMEM(object):
    ...
    def act_value(self, key, knn):
        ...
        if self.curr_capacity < knn:
            ...
            return values, self.beta/np.sqrt(counts), np.array(exact_refer)
        ...
        for i in range(len(dist)):
            ...
            if dist[i][0] < self.threshold:
                ...
                value_decay = self.external_value[ind[i][0]]+self.internal_value[ind[i][0]]
                ...
                for j, index in enumerate(ind[i]):
                    value_decay += (self.internal_value[index]+self.external_value[index]) * coeff[j]
                    count += 1 * coeff[j]
