    ...
    with U.make_session(4) as sess:
        ...
        def act_baseline(z, stochastic=0, update_eps=-1):
            ...
            if np.random.random() < max(stochastic, eps):
                ...
                v, action_prob, _, _ = ec_buffer.act_value(z, args.knn)
                if action_prob is None:
                    action_prob = np.ones((env.action_space.n,), dtype=np.float32) / env.action_space.n
                acts = -1
                act_rand = np.random.random()
                while act_rand > 0:
                    acts += 1
                    act_rand -= action_prob[acts]
                acts = min(env.action_space.n, max(0, acts))
        ...
        def update_ec(sequence):
            ...
            for seq in reversed(sequence):
                ...
                Rtds.append(Rtd)
                z = np.array(z).reshape((args.latent_dim))
                v, _, _ = ec_buffer.peek(z, Rtd, a, True)
                if v is None:  # new action
                    ec_buffer.add(z, Rtd, a)
            return Rtds


        # Create training graph and replay buffer
        act, train = deepq.build_train_dueling(
