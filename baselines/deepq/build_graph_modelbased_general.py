...
def build_train_modelbased_general(make_obs_ph, net_func, model_func, num_actions, optimizer, grad_norm_clipping=None,
                           gamma=1.0,
                           scope="mfec",
...
`       See the top of the file for details.
    ...
    z_func = build_act_modelbased_general(make_obs_ph, net_func, num_actions, scope=scope, secondary_scope="net_func",
                                  reuse=reuse)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        z_mc_pos = tf.reshape(z_mc_pos, [-1, latent_dim])
        ...
        pos_dist = tf.reduce_sum(tf.square(z_mc - z_mc_pos), axis=1)
        neg_dist = tf.reduce_mean(tf.reduce_sum(tf.square(z_mc_tile - z_mc_neg), axis=2), axis=1)
        contrast_loss = tf.reduce_mean(tf.maximum(pos_dist - neg_dist + 1.414, 0)) + tf.reduce_mean(
            tf.square(pos_dist - 1))
        ...
        total_loss = 10*contrast_loss + transition_loss + reward_loss
        # total_loss = contrast_loss
        ...
        trivial_loss_summary = tf.summary.scalar("trivial loss",
                                                 tf.reduce_sum(tf.square(z_mc_model_t - z_mc_model_tp1)))
