...
def build_train_contrast_target(input_type, obs_shape, model_func, num_actions, optimizer, grad_norm_clipping=None,
                                gamma=1.0,
                                scope="mfec",
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        obs_input_negative = U.ensure_tf_input(input_type(obs_shape, batch_size, name="enc_obs_neg"))
        ...
        # reward_mask = tf.cast(tf.equal((reward_tile - tf.transpose(reward_tile)), tf.constant(0.)), tf.float32)
